...
import os, re, time, glob, zipfile, json
...
def gather_val_res(dataset_dir:Optional[str]=None) -> Tuple[pd.DataFrame, np.ndarray, np.ndarray, List[str]]:
    """
    """
    ...
    cols = [
        "record",
        "truth", "final_pred", "final_pred_new", "rr_pred", "seq_pred",
        "truth_cls", "final_pred_cls", "final_pred_new_cls", "rr_pred_cls", "seq_pred_cls",
    ]
...
def plot_confusion_matrix(cm:np.ndarray, classes:Sequence[str],
    ...
    fig.tight_layout()
    # plt.show()
    plt.savefig(os.path.join(_VAL_RES_DIR, save_name), format=fmt, dpi=1200, bbox_inches="tight")
